 ...
 * agentic ToolLoopAgent iterates search→drill→re-search→search-sources,
 * but a fast direct-search adapter is wired when no LLM is available
 * (tests, no OPEN_ROUTER_API_KEY). Whichever implementation runs, this
 * function guarantees the empty-question case still hands the synthesizer
 * a concrete sample of what the wiki covers, so the model can compose a
 * "I couldn't match that, here's what's in here" reply instead of
 * dead-ending.
 *
 * The agent loop ALWAYS runs — accuracy beats latency for this product,
 * and an earlier direct-search fast-path skipped queries where the
 * agent's source-text fallback could have surfaced strong matches the
 * page-only direct search missed. Keeping the fallback here (rather than
 * inside each Researcher impl) means the agent stays focused on the
 * search problem and the dispatcher never sees an output without either
 * findings OR suggestionPages.
 */
