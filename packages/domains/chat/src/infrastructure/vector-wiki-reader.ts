/**
 * Vectorize-backed `searchSources` adapter. Wraps the existing token-overlap
 * `WikiReader` and short-circuits its `searchSources` method through a
 * semantic-search path:
 *
 *   1. Embed the query via `text-embedding-3-small` (1536-dim).
 *   2. Hit Cloudflare Vectorize (`tenex-sources` index, cosine metric).
 *   3. Map the returned chunk ids back to D1 `sources` rows + citing-page
 *      lookups to mint `SourceSearchHit`s in the same shape the agent loop
 *      already consumes.
 *
 * Compose-or-fallback: if either the embedder errors (missing key, network
 ...
 * Every other `WikiReader` method (`searchPages`, `getPage`, …) is a
 * straight passthrough — the seam is source search only, per scope.
 *
 * The `indexSource` method is a write-path: given a source's extracted
 * text, it chunks into 1000-char windows with 200-char overlap, embeds
 * each chunk, and upserts them into Vectorize keyed by
 * `<sourceId>::<chunkStart>`. Wikis index sources lazily — kicked off via
 * `ctx.waitUntil` from the dev-only `/__dev/reindex-sources` endpoint so
 * compilation isn't blocked on embedding throughput.
 */
...
export interface VectorWikiReader extends WikiReader {
   ...
   * (`<sourceId>::<chunkStart>`) so a re-index overwrites in place. The
   * `wikiId` is stamped onto each chunk's metadata so the query path can
   * filter to "sources cited by *this* wiki" (we still join through D1
   * post-query rather than relying on the metadata filter, which is
   * supported but adds binding surface we don't strictly need).
   */
  indexSource(args: {
    wikiId: WikiId;
    sourceId: SourceId;
...
export const createVectorWikiReader = (deps: VectorWikiReaderDeps): VectorWikiReader => {
  ...
  return {
    // Passthrough — semantic search is scoped to sources only.
    ...
    async searchSources(args) {
      ...
      for (const m of matches) {
        ...
        const meta = (m.metadata ?? {}) as Partial<ChunkMetadata>;
