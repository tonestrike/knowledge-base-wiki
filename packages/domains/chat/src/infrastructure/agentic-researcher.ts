 ...
 *
 *   - `searchWiki({ query })` — semantic page search in production,
 *     with D1 keyword-overlap as a resilience path. Used for
 ...
 *     production, with D1/R2 keyword-overlap as a resilience path. Returns
...
- \`searchWiki\` — semantic search across compiled wiki pages in production,
  with keyword-overlap search across page titles + bodies as a resilience path.
- \`readWikiPage\` — fetch a page's full body + citations by id.
- \`searchSources\` — semantic source-chunk search in production, with
  keyword-overlap search over the raw source documents (PDFs / markdown the
  wiki was compiled from) as a resilience path. Returns the wiki pages that
  cite each matching source.
