...
| Vectorize | Cloudflare's managed vector index binding. Backs the semantic-search fallback path for `WikiReader.searchSources` — query text is embedded via OpenAI and the top-K nearest chunks are mapped back to `Source` rows. The composition root injects a `VectorWikiReader` that wraps the existing token-overlap reader; when the binding (or the embedding key) is missing, search falls through to the token-overlap implementation with zero behavior change. | The index name in production is `tenex-sources`; dimensionality is 1536 (matching `text-embedding-3-small`); the metric is cosine. |
| embedding | A dense vector representation of a `Source` chunk or query string. Sources are chunked into 1000-char windows with 200-char overlap and embedded once at indexing time; queries are embedded per chat turn. Embeddings never replace `Citation`s — the fabrication tripwire remains the only path that mints them. | Provided by the OpenAI `text-embedding-3-small` model via the injected `Embedder` port. |
| cosine | The distance metric the `Vectorize` index uses to rank chunk embeddings against the query embedding. | Set at index-creation time; changing it requires recreating the index. |
| OpenAI | The provider of the embeddings model used by the `embedding` path. Distinct from the `Synthesizer` and `Researcher`, which run on Anthropic models via OpenRouter — only the embedder talks to OpenAI directly. | Reads `OPENAI_EMBEDDING_API_KEY` from the Worker bindings. |
