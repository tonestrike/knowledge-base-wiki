...
| Vectorize | Cloudflare's managed vector index binding for semantic retrieval. Backs BOTH `WikiReader.searchSources` AND `WikiReader.searchPages` (the agent's `searchWiki` tool): query text is embedded via OpenRouter, nearest records are ranked by vector similarity, and matches are hydrated back into `Source` rows or `WikiPage`s by metadata kind. If Vectorize or the embedder is unavailable, the reader uses plain keyword-overlap search so chat can still return D1/R2-backed results. | The index name in production is `tenex-sources`; dimensionality is 1536 (matching `text-embedding-3-small`); the metric is cosine. Each vector carries a `kind: 'source' \| 'page'` metadata discriminator. |
