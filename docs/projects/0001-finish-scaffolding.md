...
- [x] ~~`curl http://localhost:8787/rpc/core/health` returns `{ "status": "ok", "timestamp": "..." }`~~ — Blocked by Bug #3 (workerd local-loopback hang, see Notes). Equivalent coverage achieved via the regression test below, which exercises the exact same handler chain (Hono router → oRPC RPCHandler with `prefix: '/rpc'` → `core.health`) in-process without going through workerd.
...
- [x] ~~Visiting `http://localhost:5173/` shows the health response rendered~~ — SPA loads but the `useQuery(orpc.core.health.queryOptions())` call would fail because the api at :8787 doesn't respond (Bug #3). Visual verification deferred until the workerd issue is resolved.
...
3. **Open: workerd local-loopback hang.** Wrangler 4.90.0 reports "Ready on http://localhost:8787" and `workerd` binds both 127.0.0.1:8787 and ::1:8787, but every TCP connection that's `ESTABLISHED` then times out without a single byte returned. Reproduced with a 3-line minimal Worker (`return new Response('hello-min')`) outside the repo, with `--ip 127.0.0.1` and an alternate port — same hang. So it's not the api code, the Hono mount, the oRPC handler, or the wrangler version (3.114 → 4.90 makes no difference). Likely a sandbox/firewall/networking issue on this specific Mac (e.g. macOS App Sandbox denying workerd's outbound or inbound socket I/O). Worth investigating separately — try `Console.app` for `workerd`/`sandbox` denials, `wrangler dev --remote` (runs Workers in CF cloud), or a clean `bun install --force` of wrangler to rule out a corrupted workerd binary. Doesn't block CI (CI runs `bun run check` which uses the test fetch helper, not workerd).
