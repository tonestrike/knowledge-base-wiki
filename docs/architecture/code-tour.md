...
| Tool              | What it does                                                                                       |
|-------------------|----------------------------------------------------------------------------------------------------|
| `searchWiki`      | Semantic Vectorize search over compiled wiki pages when configured; falls back to D1 token-overlap over page title + body |
| `readWikiPage`    | Fetch a page's full body + citations by id                                                         |
| `searchSources`   | Semantic Vectorize search over cited source chunks when configured; falls back to raw source token search and returns citing pages |
...
## 8. Where to start the code tour
...
3. [`compile-folder.ts:148` `compileFolder`](../../packages/domains/wiki/src/application/compile-folder.ts#L148)
   — start with the short stage map, then drill into the internal pipeline below it
