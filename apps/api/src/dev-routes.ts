...
export const mountDevRoutes = (app: Hono<any>): void => {
  ...
  // (Vectorize backfill moved to `apps/api/src/admin-routes.ts` —
  // `POST /__admin/reindex-wiki/:wikiId`. The dev-only endpoint that
  // lived here couldn't be reached in production because of the env
  // gate; the admin route is session-cookie-gated so it works in
  // prod for the operator without exposing the index to anonymous
  // callers.)
